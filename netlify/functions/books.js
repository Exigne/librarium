...
exports.handler = async (event) => {
  if (!process.env.DATABASE_URL) {
    return { statusCode: 500, body: JSON.stringify({ error: "DATABASE_URL environment variable is missing in Netlify." }) };
  }
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      // Fetch metadata only (no heavy file content) to keep the library view fast
    ...
    if (event.httpMethod === 'POST') {
      ...
      return { statusCode: 201, body: JSON.stringify({ id: res.rows[0].id, message: 'Book uploaded successfully' }) };
    }
